...
if missing_params.any?
  ...
  missing_params.each do |param_info|
    ...
    # Extract the parameter definition from parameters.yaml
    param_pattern = /^#{Regexp.escape(param_name)}:[ ]?\n((?:  .+\n)*)/
    param_match = params_content.match(param_pattern)
    ...
    # Get the parameter content (2-space indent in source)
    param_content = param_match[0]
    
    # For components.parameters, we need:
    # - Parameter name at 4-space indent (was at column 0, add 4)
    # - Parameter content at 6-space indent (was at 2-space, add 4 to get 6)
    # Strategy: Add 4 spaces to all lines
    indented_param = param_content.lines.map { |line|
      line.strip.empty? ? line : "    #{line}"
    }.join
    
    # Convert external $ref to internal format
    indented_param = indented_param.gsub(
      /\$ref:\s*['"]?#\/([^'"]+)['"]?/,
      '$ref: \'#/components/parameters/\1\''
    )
    
    # Find parameters section and insert after the header
    params_section_match = api_content.match(/^  parameters:\s*\n/)
    insert_pos = params_section_match.end(0)
    
    # Insert the parameter
    api_content.insert(insert_pos, indented_param)
    
    modifications[:added] << param_name
    puts "  ✅ Added: #{param_name}"
  end
end
